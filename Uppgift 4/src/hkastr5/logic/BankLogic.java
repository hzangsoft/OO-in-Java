...
public class BankLogic implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5678219127531103525L;
	...
	private void createTestData() {
		...
		pNo = "8505221898";
		a = this.createAccount(pNo,AccountTypes.CREDITACCOUNT);
		...
		a = this.createAccount(pNo,AccountTypes.SAVINGSACCOUNT);
	 ...
	 */
	public int createAccount(String pNo, AccountTypes a) {
		int index = getCustomerIndex(pNo);
		int result = -1;
		// Kontrollera om kunden finns
		if (index >= 0) {
			switch (a) { 
			case SAVINGSACCOUNT: {
				result = customerList.get(index).createSavingsAccount();
				break;
			}
			case CREDITACCOUNT: {
				result = customerList.get(index).createCreditAccount();
				break;
			}
